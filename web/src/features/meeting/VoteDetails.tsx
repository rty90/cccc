...
export function VoteDetails({ vote, meeting, isDark }: { vote: Vote; meeting: Meeting; isDark: boolean }) {
  ...
  const linkClass = "knots-press rounded-full border border-[var(--glass-border-subtle)] bg-[var(--glass-tab-bg)] px-2 py-0.5 text-[10px] font-medium text-[var(--color-text-secondary)]";
  return (
    <div className="mt-1 text-[11px]">
      ...
      {showBrief ? (
        <pre className={classNames("mt-1 max-h-64 overflow-auto whitespace-pre-wrap rounded-lg p-2 text-[11px] leading-4", isDark ? "bg-white/5" : "bg-black/[0.04]")}>{meeting.brief}</pre>
      ) : null}
      {showBallots ? (
