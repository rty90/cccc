...
export function HelpTicketCard({ ticket, actorIds, isDark, compact }: { ticket: HelpTicket; actorIds: string[]; isDark: boolean; compact?: boolean }) {
  ...
  const buttonClass = "knots-press rounded-full border border-[var(--glass-border-subtle)] bg-[var(--glass-tab-bg)] px-2 py-0.5 text-[11px] font-medium disabled:opacity-50";
  return (
    <div className={classNames("rounded-xl border p-2", isDark ? "border-white/10 bg-white/[0.03]" : "border-black/8 bg-black/[0.02]")}>
      <div className="flex flex-wrap items-center gap-1.5 text-[12px]">
        ...
        <span className={classNames("rounded-full px-2 py-0.5 text-[11px] font-semibold", needTone)}>{t(`helpNeed_${ticket.need}`)}</span>
        <span className="rounded-full bg-black/5 px-2 py-0.5 text-[11px] dark:bg-white/10">{t(`helpStatus_${ticket.status}`)}{ticket.assignee ? ` → ${ticket.assignee}` : ""}</span>
        <span className="ml-auto text-[11px] text-[var(--color-text-tertiary)]">{String(ticket.created_at).slice(11, 16)}</span>
      </div>
      <div className={classNames("mt-1 whitespace-pre-wrap text-[12px] text-[var(--color-text-secondary)]", full ? "" : compact ? "line-clamp-3" : "line-clamp-6")}>{ticket.text}</div>
      ...
      {ticket.resolution ? (
        <div className="mt-1 rounded-lg bg-violet-500/10 px-2 py-1 text-[12px]">
      ...
      {open ? (
        <div className="mt-2 space-y-1.5">
          <div className="flex flex-wrap items-center gap-1">
            <span className="text-[11px] text-[var(--color-text-tertiary)]">{t("helpAssignTo")}</span>
            {actorIds
