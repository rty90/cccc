      ...
      <div className="flex min-h-0 flex-1 flex-col gap-4">
        ...
        >
          <div className={classNames("flex flex-wrap gap-1")}>
            ...
            >
              {tr("context.activityLog", "Activity")}
            </button>
          </div>
                    ...
                    <>
                      ...
                      >
                        {syncBusy
                          ...
                          : tr("context.saveBrief", "Save summary")}
                      </button>
                    </>
                  ) : (
                    <button
              ...
              {editingBrief ? (
                <div className="mt-4 grid gap-3 lg:grid-cols-2">
                  <label className="block text-sm lg:col-span-1">
                    ...
                    >
                      {tr("context.objective", "Objective")}
                    </span>
                    <input
                      value={briefDraft.objective}
                      onChange={(event) =>
                        onBriefDraftChange((prev) => ({ ...prev, objective: event.target.value }))
                      }
                  ...
                  <label className="block text-sm lg:col-span-1">
                    ...
                    <input
                      value={briefDraft.currentFocus}
                      onChange={(event) =>
                        onBriefDraftChange((prev) => ({
                          ...prev,
                  ...
                  <label className="block text-sm lg:col-span-2">
                    ...
                    <textarea
                      ...
                      onChange={(event) =>
                        onBriefDraftChange((prev) => ({ ...prev, constraints: event.target.value }))
                      }
                      className={classNames(ui.textareaClass, "min-h-[110px]")}
                      placeholder={tr("context.constraintsPlaceholder", "One per line")}
                    />
                  </label>
                  <label className="block text-sm lg:col-span-2">
                    <span
                    ...
                    <textarea
                      ...
                      className={classNames(ui.textareaClass, "min-h-[180px]")}
                    />
                  </label>
                  <label
                    className={classNames(
                      "flex items-center gap-2 rounded-lg border px-3 py-2 text-sm lg:col-span-2",
                      "glass-card text-[var(--color-text-primary)]",
                    )}
                  >
                    <input
                      type="checkbox"
                      checked={briefDraft.projectBriefStale}
                      onChange={(event) =>
                        onBriefDraftChange((prev) => ({
                          ...prev,
                          projectBriefStale: event.target.checked,
                        }))
                      }
                    />
            ...
            <div className="space-y-3">
              ...
              <div className="grid gap-3 xl:grid-cols-2">
                <section className={classNames("rounded-xl border p-4", "glass-card")}>
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      >
                        {tr("context.recentDecisions", "Recent decisions")}
                      </div>
                      <div className={classNames("mt-1 text-xs", ui.mutedTextClass)}>
                        {tr(
                          "context.activityWriteHint",
                          "Capture durable decisions and handoffs here when chat alone is too transient.",
                        )}
                      </div>
                    </div>
                  </div>
                  <div className={classNames("mt-3 rounded-xl border p-3", "glass-card")}>
                    <textarea
                      ...
                      onChange={(event) =>
                        onDecisionDraftChange((prev) => ({ ...prev, summary: event.target.value }))
                      }
                      className={classNames(ui.textareaClass, "min-h-[96px]")}
                      placeholder={tr(
                        "context.decisionPlaceholder",
                        "Record a durable decision or constraint...",
                      )}
                    />
                    <div className="mt-2 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
                      <SelectCombobox
                        ...
                        onChange={(value) =>
                          onDecisionDraftChange((prev) => ({ ...prev, taskId: value }))
                        }
                      ...
                      <button
                        type="button"
                        onClick={() => onAddCoordinationNote("decision")}
                        disabled={
                          activityBusyKind !== null || !String(decisionDraft.summary || "").trim()
                        }
                        className={ui.buttonPrimaryClass}
                      >
                        {activityBusyKind === "decision"
                          ? tr("context.saving", "Saving…")
                          : tr("context.addDecision", "Add decision")}
                      </button>
                    </div>
                  </div>
                  <div className="mt-3 space-y-2">
                    {recentDecisions.length > 0 ? (
                      recentDecisions.map((note, index) => (
                        <div key={`${note.summary}-${index}`} className={notesCardClass}>
                          ...
                          <div className={classNames("mt-1 text-xs", ui.mutedTextClass)}>
                            {[note.by || "", note.task_id || "", noteTimestamp(note)]
                              .filter(Boolean)
                              .join(" · ") || tr("context.noMetadata", "No metadata")}
                          </div>
                        </div>
                      ))
                    ) : (
                      <div className={classNames("text-sm", ui.mutedTextClass)}>
                        {tr("context.noRecentDecisions", "No recent decisions")}
                      </div>
                    )}
                  </div>
                </section>
                <section className={classNames("rounded-xl border p-4", "glass-card")}>
                  ...
                  >
                    {tr("context.recentHandoffs", "Recent handoffs")}
                  </div>
                  <div className={classNames("mt-3 rounded-xl border p-3", "glass-card")}>
                    <textarea
                      value={handoffDraft.summary}
                      onChange={(event) =>
                        onHandoffDraftChange((prev) => ({ ...prev, summary: event.target.value }))
                      }
                      className={classNames(ui.textareaClass, "min-h-[96px]")}
                      placeholder={tr(
                        "context.handoffPlaceholder",
                        "Record a durable handoff or next-owner note...",
                      )}
                    />
                    <div className="mt-2 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
                      <SelectCombobox
                        ...
                        onChange={(value) =>
                          onHandoffDraftChange((prev) => ({ ...prev, taskId: value }))
                        }
                        ariaLabel={tr("context.noRelatedTask", "No related task")}
                        className={ui.inputClass}
                        searchable
                      ...
                      <button
                        type="button"
                        onClick={() => onAddCoordinationNote("handoff")}
                        disabled={
                          activityBusyKind !== null || !String(handoffDraft.summary || "").trim()
                        }
                        className={ui.buttonPrimaryClass}
                      >
                        {activityBusyKind === "handoff"
                          ? tr("context.saving", "Saving…")
                          : tr("context.addHandoff", "Add handoff")}
                      </button>
                    </div>
                  </div>
                  <div className="mt-3 space-y-2">
                    {recentHandoffs.length > 0 ? (
                      recentHandoffs.map((note, index) => (
                        <div key={`${note.summary}-${index}`} className={notesCardClass}>
                          <div
                            className={classNames(
                              "font-medium",
                      ...
                      <div className={classNames("text-sm", ui.mutedTextClass)}>
                        {tr("context.noRecentHandoffs", "No recent handoffs")}
                      </div>
                    )}
                  </div>
                </section>
