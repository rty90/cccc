...
fn profile_paths_escape_yaml_apostrophes() {
    ...
    write_profile_files(&profile, &executable, cccc_contracts::deepseek::DEEPSEEK_DEFAULT_MODEL, cccc_contracts::deepseek::DEEPSEEK_DEFAULT_REASONING).expect("profile");
...
fn upgrades_legacy_bundle_root_and_removes_obsolete_profile_patch() {
    ...
    let profile = dsh_home.join("profiles/cccc-acp");
    write_profile_files(&profile, &cccc_executable(temp.path()), cccc_contracts::deepseek::DEEPSEEK_DEFAULT_MODEL, cccc_contracts::deepseek::DEEPSEEK_DEFAULT_REASONING).expect("managed profile");
